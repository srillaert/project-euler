...
fn get_maximum_path_sum(numbers: &mut Vec<u8>) -> u16 {
    let row_count = get_row_count(numbers.len());
    let mut maximum_path_row: Vec<_> = ((numbers.len() - row_count).. numbers.len()).map(|i| numbers[i] as u16).collect();
    for row_index in (0..(row_count - 1)).rev() {
        for column_index in 0..=row_index {
            ...
            maximum_path_row[column_index] = 
                numbers[row_start + column_index] as u16 + 
...
fn test_get_maximum_path_sum() {
    ...
    let actual = get_maximum_path_sum(&mut numbers);
...
fn main() {
    ...
    let solution = get_maximum_path_sum(&mut numbers);
    println!("{}", solution);
}